...
base_GETCUR() {
    X=$(grep '_VERSION = ' $(find package/retrolx -name "${1}.mk") 2>/dev/null | grep -vE '^#' | head -1 | sed -e s+'.* = '+''+ | sed -e s+' '++g)
    if test -z "$X"
    then
...
github_base() {
    ...
    then
	grep '_SITE = \$(call github,' $(find package/retrolx -name "${1}.mk") 2>/dev/null | grep -vE '^#' | head -1 | sed -e s+'^.*call github,\([^,]*\),\([^,]*\),.*$'+'\1/\2:lastcommit'+
    else
	grep '_SITE = \$(call github,' $(find package/retrolx -name "${1}.mk") 2>/dev/null | grep -vE '^#' | head -1 | sed -e s+'^.*call github,\([^,]*\),\([^,]*\),.*$'+'\1/\2:'"${GH_VERS}"+
    fi
}
...
setPGroups() {
	...
	PACKAGES=$(find package/retrolx -name "*.mk" | grep -vE '/retrolx\.mk$' | sed -e s+"^.*/\([^/]*\).mk$"+"\1"+ | tr '\n' ' ')
	return
    fi
...
base_UPDATE() {
    sed -i -e s+"^\([ ]*[a-zA-Z0-9_]*_VERSION[ ]*=[ ]*\).*$"+"\1${2}"+ $(find package/retrolx -name "${1}.mk")
}

run_update() {
    updpkg=${1}

    if test ! -f $(find package/retrolx -name "${1}.mk")
    then
