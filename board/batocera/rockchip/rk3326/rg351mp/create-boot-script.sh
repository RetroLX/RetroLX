...
mkdir -p "${BATOCERA_BINARIES_DIR}/build-uboot-rg351mp" || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${BATOCERA_BINARIES_DIR}/build-uboot-rg351mp/" || exit 1
cd "${BATOCERA_BINARIES_DIR}/build-uboot-rg351mp/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/rk3326-rg351mp.dtb"         "${BATOCERA_BINARIES_DIR}/boot/boot/rk3326-rg351mp.dtb"         || exit 1
