...
cat "${BUILD_DIR}/board/retrolx/packages.txt" "${BUILD_DIR}/board/retrolx/allwinner/sun50i/packages.txt" > "${RETROLX_BINARIES_DIR}/boot/packages.txt"
...
# ATF
"${BR2_EXTERNAL_RETROLX_PATH}/board/retrolx/scripts/build-atf.sh" "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" sun50i_a64
...
cp "${BINARIES_DIR}/kernel-sun50i/sun50i-h5-libretech-all-h3-cc.dtb" "${RETROLX_BINARIES_DIR}/boot/boot/"     || exit 1
