...
# U-Boot
mkdir -p "${RETROLX_BINARIES_DIR}/build-uboot-gameforce" || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${RETROLX_BINARIES_DIR}/build-uboot-gameforce/" || exit 1
cd "${RETROLX_BINARIES_DIR}/build-uboot-gameforce/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
