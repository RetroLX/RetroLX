...
# U-Boot
mkdir -p "${RETROLX_BINARIES_DIR}/build-uboot-khadas-vim3l"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${RETROLX_BINARIES_DIR}/build-uboot-khadas-vim3l/" || exit 1
cd "${RETROLX_BINARIES_DIR}/build-uboot-khadas-vim3l/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/kernel-s905gen3/meson-sm1-khadas-vim3l.dtb"        "${RETROLX_BINARIES_DIR}/boot/boot/"     || exit 1
