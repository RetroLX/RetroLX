...
#"${BR2_EXTERNAL_RETROLX_PATH}/board/retrolx/scripts/build-atf.sh" "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" sm1

# U-Boot
mkdir -p "${RETROLX_BINARIES_DIR}/build-uboot-odroid-c4"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${RETROLX_BINARIES_DIR}/build-uboot-odroid-c4/" || exit 1
cd "${RETROLX_BINARIES_DIR}/build-uboot-odroid-c4/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/kernel-s905gen3/meson-sm1-odroid-c4.dtb"           "${RETROLX_BINARIES_DIR}/boot/boot/"     || exit 1
