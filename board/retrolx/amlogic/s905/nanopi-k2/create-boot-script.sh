...
# ATF
"${BR2_EXTERNAL_RETROLX_PATH}/board/retrolx/scripts/build-atf.sh" "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" gxbb

# U-Boot
mkdir -p "${RETROLX_BINARIES_DIR}/build-uboot-nanopi-k2"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${RETROLX_BINARIES_DIR}/build-uboot-nanopi-k2/" || exit 1
cd "${RETROLX_BINARIES_DIR}/build-uboot-nanopi-k2/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/kernel-meson64/meson-gxbb-nanopi-k2.dtb" "${RETROLX_BINARIES_DIR}/boot/boot/"     || exit 1
