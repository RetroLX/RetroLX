...
# U-Boot
mkdir -p "${RETROLX_BINARIES_DIR}/build-uboot-odroid-c2"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${RETROLX_BINARIES_DIR}/build-uboot-odroid-c2/" || exit 1
cd "${RETROLX_BINARIES_DIR}/build-uboot-odroid-c2/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/kernel-meson64/meson-gxbb-odroidc2.dtb" "${RETROLX_BINARIES_DIR}/boot/boot/"     || exit 1
