...
define RETROLX_SHADERS_INSTALL_TARGET_CMDS
	...
	# general
	cp $(RETROLX_SHADERS_DIRIN)/rendering-defaults.yml           $(TARGET_DIR)/usr/share/batocera/shaders/configs/
	if test -e $(RETROLX_SHADERS_DIRIN)/rendering-defaults-$(RETROLX_SHADERS_SYSTEM).yml; then \
		cp $(RETROLX_SHADERS_DIRIN)/rendering-defaults-$(RETROLX_SHADERS_SYSTEM).yml $(TARGET_DIR)/usr/share/batocera/shaders/configs/rendering-defaults-arch.yml; \
	fi
	...
	for set in retro scanlines enhanced curvature zfast flatten-glow; do \
		...
		cp $(RETROLX_SHADERS_DIRIN)/$$set/rendering-defaults.yml     $(TARGET_DIR)/usr/share/batocera/shaders/configs/$$set/; \
		if test -e $(RETROLX_SHADERS_DIRIN)/$$set/rendering-defaults-$(RETROLX_SHADERS_SYSTEM).yml; then \
			cp $(RETROLX_SHADERS_DIRIN)/$$set/rendering-defaults-$(RETROLX_SHADERS_SYSTEM).yml $(TARGET_DIR)/usr/share/batocera/shaders/configs/$$set/rendering-defaults-arch.yml; \
		fi \
	done
