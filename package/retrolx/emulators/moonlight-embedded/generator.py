...
class MoonlightGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, gameResolution):
        ...
        outputFile = retrolxFiles.moonlightCustom + '/gamecontrollerdb.txt'
        ...
        commandArray = [retrolxFiles.batoceraBins[system.config['emulator']], 'stream','-config',  confFile]
        commandArray.append('-app')
        commandArray.append(gameName)
        return Command.Command(array=commandArray, env={"XDG_DATA_DIRS": retrolxFiles.CONF})

    def getRealGameNameAndConfigFile(self, rom):
        # Rom's basename without extension
        romName = os.path.splitext(os.path.basename(rom))[0]
        # find the real game name
        f = open(retrolxFiles.moonlightGamelist, 'r')
        gfeGame = None
        for line in f:
            try:
                ...
                confFile = retrolxFiles.moonlightConfigFile
            #If found
            if gfeRom == romName:
                # return it
                f.close()
        ...
        return [gfeGame, retrolxFiles.moonlightConfigFile]
